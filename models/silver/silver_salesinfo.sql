...
products as 
(
    select product_sk, category from 
    {{ ref('bronze_product') }} 
),
...
joined_query as (
select 
sales.sales_id, sales.gross_amount, sales.payment_method,
products.category, customer.gender 
from sales
...
from joined_query group by 
category, gender order by total_sales desc 